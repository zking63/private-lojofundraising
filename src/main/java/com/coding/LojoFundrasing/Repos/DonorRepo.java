...
public interface DonorRepo extends CrudRepository<Donor, Long>{
	...
	//date functions
	@Query(value = "SELECT * FROM donors LEFT JOIN committees ON committees.id = donors.committees_id WHERE committees.id = :committee_id AND most_recent_datein_range >= DATE(:startdate) and most_recent_datein_range < DATE_ADD(DATE(:enddate), INTERVAL 1 DAY) order by mostrecent_date Desc", nativeQuery = true)
